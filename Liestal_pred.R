...
Liestal_bloom = read.csv("/Users/R.studio/GMU-CherryBlossomCompetition/liestal.csv")
...
#Filter Liestal
Liestal_weather = subset(weather_data, STATION == "SZ000001940" & !is.na(TMEAN))
...
for (base in 35:45) {
  ...
  Liestal_weather$GDD_temp = pmax(0, Liestal_weather$TMEAN - base)
  Liestal_weather$Freeze_temp = ifelse(Liestal_weather$TMIN < 32, 1, 0)
  
  # Historical features (up to 2025)
  hist_temp = subset(Kyoto_weather, Year >= min(Liestal_bloom$year) & Year < 2026)
  ...
  train_data = merge(Kyoto_bloom[, c("year", "bloom_doy")], gdd_wide, by.x="year", by.y="Year")
...
if (is.null(best_model)) {
  final_doy = round(mean(Liestal_bloom$bloom_doy))
  ...
  Liestal_weather$GDD = pmax(0, Liestal_weather$TMEAN - best_base)
  Liestal_weather$Freeze = ifelse(Liestal_weather$TMIN < 32, 1, 0)
  
  actual_2026 = subset(Liestal_weather, Year == 2026)
  recent_past = subset(Kyoto_weather, Year >= 2015 & Year <= 2024)
