...
Kyoto_bloom = read.csv("/Users/R.studio/GMU-CherryBlossomCompetition/kyoto.csv")
...
#Filter NYC
Kyoto_weather = subset(weather_data, STATION == "JA000047759" & !is.na(TMEAN))
...
for (base in 35:45) {
  ...
  Kyoto_weather$GDD_temp = pmax(0, Kyoto_weather$TMEAN - base)
  Kyoto_weather$Freeze_temp = ifelse(Kyoto_weather$TMIN < 32, 1, 0)
  
  # Historical features (up to 2025)
  hist_temp = subset(Kyoto_weather, Year >= min(Kyoto_bloom$year) & Year < 2026)
...
if (is.null(best_model)) {
  final_doy = round(mean(Kyoto_bloom$bloom_doy))
  ...
  Kyoto_weather$GDD = pmax(0, Kyoto_weather$TMEAN - best_base)
  Kyoto_weather$Freeze = ifelse(Kyoto_weather$TMIN < 32, 1, 0)
  
  actual_2026 = subset(Kyoto_weather, Year == 2026)
