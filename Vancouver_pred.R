...
vancouver_bloom = read.csv("/Users/R.studio/GMU-CherryBlossomCompetition/vancouver.csv")
...
#Vancouver
vancouver_weather = subset(weather_data, STATION = "CA001108395" & !is.na(TMEAN))
...
for (base in 35:45) {
  ...
  vancouver_weather$GDD_temp = pmax(0, vancouver_weather$TMEAN - base)
  vancouver_weather$Freeze_temp = ifelse(vancouver_weather$TMIN < 32, 1, 0)
  
  # Historical features (up to 2025)
  hist_temp = subset(vancouver_weather, Year >= min(vancouver_bloom$year) & Year < 2026)
...
if (is.null(best_model)) {
  final_doy = round(mean(vancouver_bloom$bloom_doy))
  ...
  vancouver_weather$GDD = pmax(0, vancouver_weather$TMEAN - best_base)
  vancouver_weather$Freeze = ifelse(vancouver_weather$TMIN < 32, 1, 0)
  
  actual_2026 = subset(vancouver_weather, Year == 2026)
  recent_past = subset(vancouver_weather, Year >= 2015 & Year <= 2024)
